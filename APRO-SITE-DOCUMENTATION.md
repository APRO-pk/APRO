...
- Student applications auto-create auth accounts, then submit through the authenticated `submit_student_application` RPC
- Student member, application, and detail records are created atomically and incomplete Auth accounts can safely retry
- Member login resolves only the current user's profile through `get_my_member_account`, without requiring broad `members` table access
